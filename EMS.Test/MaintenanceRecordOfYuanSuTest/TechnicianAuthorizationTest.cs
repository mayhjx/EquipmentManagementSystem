...
namespace EMS.Test.MaintenanceRecordOfYuanSuTest
{
    public class TechnicianAuthorizationTest
    {
        ...
        [Fact]
        public async Task Handler_UpdateMaintenanceRecordOfYuanSuWithOwningGroup_ShouldSucceed()
        {
            // 同一项目组内，技术员可以修改他人新建的记录
            var resource = new MaintenanceRecordOfYuanSu { Operator = "Test 1", GroupName="Group" };
        ...
        [Fact]
        public async Task Handler_UpdateMaintenanceRecordOfYuanSuWithIllegalUser_ShouldFail()
        {
            // 技术员不可以修改其他项目组人员新建的记录
            var resource = new MaintenanceRecordOfYuanSu { Operator = "Test 1",GroupName="Group 1" };
            var user = new ClaimsPrincipal(new ClaimsIdentity(new List<Claim> {
                new Claim(ClaimTypes.Role, Constants.TechnicianRole),
                new Claim(ClaimTypes.GivenName, "Test 2"),
        ...
        [Fact]
        public async Task Handler_DeleteMaintenanceRecordOfYuanSuWithOwner_ShouldFail()
        {
            var resource = new MaintenanceRecordOfYuanSu { Operator = "Test" };
            var user = new ClaimsPrincipal(new ClaimsIdentity(new List<Claim> {
                new Claim(ClaimTypes.Role, Constants.TechnicianRole),
        ...
        [Fact]
        public async Task Handler_DeleteMaintenanceRecordOfYuanSuWithIllegalUser_ShouldFail()
        {
            var resource = new MaintenanceRecordOfYuanSu { Operator = "Test 1" };
            var user = new ClaimsPrincipal(new ClaimsIdentity(new List<Claim> {
                new Claim(ClaimTypes.Role, Constants.TechnicianRole),
                new Claim(ClaimTypes.GivenName, "Test 2"),
            }));
